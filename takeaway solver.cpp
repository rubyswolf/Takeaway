...
int main()
{
	...
	int* totalNodes = new int(0); // Prepare a counter for how many nodes we generate
	...
	int* totalPrunedNodes = new int(0); // Prepare a counter for how many nodes we prune
	...
	*totalPrunedNodes = 0; // Reset the counter
	...
	std::ofstream(gameListFileName) << gameTree.generateGameList(false); // Generate a and save a list of all the games where the player that can always win plays perfectly to always win
	...
	std::ofstream(treeFileName) << gameTree.generateTreeDiagram(); // Generate and save a text based diagram of the final pruned game tree to visualize the winning strategy for the player that always wins
