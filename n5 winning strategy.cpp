...
MoveTest triple_move = legal & any_from(3, pass);
MoveTest pair_move = legal & any_from(2, pass);
MoveTest compliment_move = all_elements(~picked_on_move(previous_move));

MoveTest singleton_move = legal & any_from(1, pass);
...
IF (full_coverage.ever_after(singleton_move)) {
    PICK(singleton_move & causes(full_coverage), "Strong reduction");
}
